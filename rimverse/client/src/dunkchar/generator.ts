// generator.ts — procedural NBA Jam–style big-head pixel-art sprite sheets.
//
...
//
// Pixel-identical TypeScript port of v3's public/js/generator.js. The golden
// fidelity test (client/test/dunkchar-fidelity.test.ts) certifies byte-for-byte
// equivalence; do not change any rendering math, constant, or draw order.

import type { Character } from '../../../shared/src/character';
...
export function generateSpriteSheet(character: Partial<Character> = {}) {
...
export function renderPreview(character: Partial<Character>, animCode = 0, frame = 0, scale = 2) {
  const cfg = withDefaults(character);
  const canvas = document.createElement('canvas');
  ...
  const ctx = canvas.getContext('2d') as unknown as CanvasRenderingContext2D;
  ctx.imageSmoothingEnabled = false;
  const rows: Record<number, () => Pose[]> = { 0: idleFrames, 1: runFrames, 2: dribbleFrames, 3: jumpFrames, 4: shootFrames, 5: dunkFrames, 6: celebrateFrames };
  const frames = (rows[animCode] || idleFrames)();
  const tmp = document.createElement('canvas');
  tmp.width = SHEET.frameW; tmp.height = SHEET.frameH;
  const tctx = tmp.getContext('2d') as unknown as CanvasRenderingContext2D;
...
function withDefaults(c: Partial<Character>): Character {
  return {
...
function runFrames(): Pose[] {
  // 6-frame stride cycle: reach → plant → push-off → trail (heel up) →
  // recovery (knee folded, foot lifted) → swing-through. Shins trail the
  // thigh (knees flex backward only) — forward-bent shins kept the recovery
  // foot at ground level and the run read as a moonwalk (fixed with v3).
  const stride = [
  ...
  return stride.map((s, i) => ({
    ...
    legF: s.f as [number, number], legB: s.b as [number, number],
    headBob: i % 3 === 1 ? 1 : 0,
  }));
}

function dribbleFrames(): Pose[] {
  const base = runFrames();
  return base.map((p, i) => ({
    ...p,
    // Front arm works the ball: pump down in front, synced to the bounce.
    armF: (i % 3 === 0 ? [50, 25] : i % 3 === 1 ? [40, 5] : [55, 35]) as [number, number],
  }));
}

function jumpFrames(): Pose[] {
  return [
...
function celebrateFrames(): Pose[] {
  return [0, 1, 2, 1, 0, 3].map((i) => ({
    crouch: i === 0 ? 2 : 0, lean: 0,
    armF: (i === 0 ? [60, 100] : [160 + i * 8, 180 + i * 8]) as [number, number],
    armB: (i === 0 ? [-60, -100] : [150 + i * 6, 170 + i * 6]) as [number, number],
    legF: (i === 0 ? [10, -15] : [20, -30]) as [number, number], legB: (i === 0 ? [-10, -12] : [-20, -25]) as [number, number],
...
function drawFigure(ctx: CanvasRenderingContext2D, cfg: Character, pose: Pose) {
...
class Grid {
  ...
  seg(x0: number, y0: number, x1: number, y1: number, t: number, color: string) {
  ...
  limb(sx: number, sy: number, [a1, a2]: [number, number], len1: number, len2: number, t: number, skin: string, skinDark: string, jersey: string, cfg: Character, isFar: boolean) {
  ...
  leg(hx: number, hy: number, [a1, a2]: [number, number], skin: string, shorts: string, shoe: string, groundY: number) {
  ...
  outlineShoe(x: number, y: number, w: number, h: number) {
    this.ctx.strokeStyle = '#101010';
    this.ctx.lineWidth = 1.5;
    this.ctx.strokeRect(Math.round(x) * PX - 0.5, Math.round(y) * PX - 0.5, Math.round(w) * PX + 1, Math.round(h) * PX + 1);
  }

  head(hx: number, hy: number, w: number, h: number, skin: string, skinDark: string, hairC: string, cfg: Character) {
    // Skull.
    ...
    // Hair.
    const styles: Record<number, () => void> = {
  ...
  number(cx: number, y: number, num: number, color: string) {
  ...
  digit(x: number, y: number, rows: number[], color: string) {
